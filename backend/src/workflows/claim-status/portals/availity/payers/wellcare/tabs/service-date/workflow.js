  ...
  await withRetry(
    ...
    async () => {
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        const frame = await getClaimStatusFrame(page);
        await throwIfVisibleFieldValidation(page, "Wellcare Service Dates");
...
async function searchWellcareServiceDatesWithProvider(page, providerName, rowData, options = {}) {
  ...
  if (options.projectId === "charm") {
    ...
    if (!providerFill?.requiresProviderDropdown) {
      throw new Error("Charm Wellcare Service Dates provider identifiers could not be filled deterministically.");
    }
  }
  ...
  if (options.projectId === "charm") {
    ...
    if (providerFillAfterDropdown?.requiresProviderDropdown) {
      throw new Error("Charm Wellcare Service Dates provider dropdown was selected, but required provider fields were still not auto-filled.");
    }
  }
  await fillServiceDateSearchForm(page, rowData);
  await submitServiceDateSearch(page);
