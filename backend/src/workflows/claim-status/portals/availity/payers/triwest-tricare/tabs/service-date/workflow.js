...
async function fillProviderTaxId(frame, taxId) {
  if (!taxId) {
    throw new Error("TRIWEST-TRICARE provider Tax ID could not be extracted from selected provider value.");
  }
...
async function selectServiceDateTab(page) {
  await withRetry(
    "Selecting TRIWEST-TRICARE Service Dates tab",
    async () => {
...
async function selectProvider(page, providerName, rowData = {}) {
  await withRetry(
    `Selecting TRIWEST-TRICARE provider ${providerName}`,
    async () => {
      const frame = await getClaimStatusFrame(page);
      const providerInput = await getProviderInput(frame);
      ...
      await verifyProviderNpiMatches(frame, providerName, { context: "TRIWEST-TRICARE Service Dates", logger });
      const providerTaxId = extractProviderTaxId(selectedProviderText || providerName) || getInputProviderTaxId(rowData);
      logger.info(`TRIWEST-TRICARE provider Tax ID extracted as "${providerTaxId || "blank"}" from provider value "${selectedProviderText || providerName}".`);
...
async function selectProviderOrFillTaxId(page, providerName, rowData = {}, options = {}) {
  ...
  if (providerAsTaxId && !groupNameOnly) {
    logger.info(`TRIWEST-TRICARE provider identifier "${providerName}" is a Tax ID. Filling Provider Tax ID directly.`);
    const frame = await clearProviderStateForTaxIdFallback(page, { context: "TRIWEST-TRICARE Service Dates Tax ID fallback", logger });
  ...
  try {
    ...
    logger.warn(`TRIWEST-TRICARE provider dropdown did not select "${providerName}". Filling Provider Tax ID "${taxId}" directly.`);
    const frame = await clearProviderStateForTaxIdFallback(page, { context: "TRIWEST-TRICARE Service Dates Tax ID fallback", logger });
...
async function submitServiceDateSearch(page) {
  ...
  await withRetry(
    "Submitting TRIWEST-TRICARE Service Dates search",
    async () => {
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        const frame = await getClaimStatusFrame(page);
        await throwIfVisibleFieldValidation(page, "TRIWEST-TRICARE Service Dates");
        ...
        logger.info(`TRIWEST-TRICARE Service Dates Search clicked (attempt ${attempt}/3). Waiting for portal response.`);
        await humanDelay(1500, 2500);

        if (await resultIndicatorAppeared(5000)) {
          logger.info(`TRIWEST-TRICARE Service Dates search response appeared after submit attempt ${attempt}.`);
          return;
        }

        if (attempt < 3) {
          logger.warn(`TRIWEST-TRICARE Service Dates search results did not appear within 5 seconds after submit attempt ${attempt}. Re-clicking Search.`);
        }
      }

      throw new Error("TRIWEST-TRICARE Service Dates Search did not produce results, no-results message, or validation response after 3 attempts.");
    },
...
async function getTriwestTricareServiceDateRows(page) {
  const frame = await getClaimStatusFrame(page);
...
function selectTriwestTricareMatchedRows(matchedRows, sourceTab) {
...
async function extractTriwestTricareMatchedRow(page, matchedRow, sourceTab) {
  logger.info(
    `Preparing to extract TRIWEST-TRICARE matched row: claim="${matchedRow.claimNumber}", status="${matchedRow.status.display}", service_date="${matchedRow.serviceDate}", billed="${matchedRow.billedAmount}", member_id="${matchedRow.memberId}"`
  );
  ...
  logger.info(`Clicked TRIWEST-TRICARE matched result row for claim ${matchedRow.claimNumber}. Waiting for detail page.`);
  await page.waitForLoadState("domcontentloaded").catch(() => {});
  await waitForClaimDetailPage(page);
  logger.success(`TRIWEST-TRICARE detail page loaded for claim ${matchedRow.claimNumber}`);
  ...
  if (matchedRow.status.type === "paid") {
    const extracted = await extractTriwestTricarePaid(page, matchedRow.status.display);
    extracted.claimNumber = extracted.claimNumber || matchedRow.claimNumber;
    return extracted;
  }

  if (matchedRow.status.type === "denied") {
    const extracted = await extractTriwestTricareDenied(page, matchedRow.status.display);
    extracted.claimNumber = extracted.claimNumber || matchedRow.claimNumber;
    return extracted;
  }
...
async function processTriwestTricareServiceDateResults(page, row, provider, resultSummary, options = {}) {
  const sourceTab = "Service Dates";
  const resultRows = await getTriwestTricareServiceDateRows(page);
  ...
  const canFallbackToPatientName = Boolean(matchingPolicy.patientNameFallback);
  ...
  if (matchedRows.length === 0 && matchingPolicy.fuzzyPatientNameFallback && hasUsableValue(row.data["Patient Name"])) {
  ...
  if (matchedRows.length === 0) {
    ...
    const mismatchReason = [
      `Portal returned ${returnedCount} rows in ${sourceTab} for provider ${provider}, but none matched input ${matchCriteria}. ${mismatchDetail} ${returnedRowsSummary}`.trim(),
  ...
  const selection = selectTriwestTricareMatchedRows(matchedRows, sourceTab);
  if (selection.notes) {
  ...
  for (let index = 0; index < selection.selectedRows.length; index += 1) {
    ...
    const extracted = await extractTriwestTricareMatchedRow(page, matchedRow, sourceTab);
...
async function searchTriwestTricareServiceDatesWithProvider(page, providerName, rowData, options = {}) {
  ...
  if (options.projectId === "charm") {
    ...
    if (!providerFill?.requiresProviderDropdown) {
      throw new Error("Charm TRIWEST-TRICARE Service Dates provider identifiers could not be filled deterministically.");
    }
  }
  ...
  if (options.projectId === "charm") {
    ...
    if (providerFillAfterDropdown?.requiresProviderDropdown) {
      throw new Error("Charm TRIWEST-TRICARE Service Dates provider dropdown was selected, but required provider fields were still not auto-filled.");
    }
  }
  await fillServiceDateSearchForm(page, rowData);
  await submitServiceDateSearch(page);
...
async function processClaim(page, row, options = {}) {
  ...
  for (const provider of providerOrder) {
    await searchTriwestTricareServiceDatesWithProvider(page, provider, row.data, options);

    logger.info(`Waiting up to 5 seconds for ${provider} TRIWEST-TRICARE Service Dates results to settle`);
    const resultSummary = await waitForSearchResultsToSettle(page, 5000);
    logger.info(
      `TRIWEST-TRICARE Service Dates provider ${provider} result summary: heading="${resultSummary.headingText || "not found"}", total=${resultSummary.total ?? "unknown"}, rows=${resultSummary.resultRowCount ?? "unknown"}, no_results_message=${resultSummary.noResultsMessageVisible}, alert="${resultSummary.portalAlertMessage || ""}"`
    );

    const resultRows = await getTriwestTricareServiceDateRows(page);
    if (resultSummary.hasPortalAlert && resultRows.length === 0) {
      logger.warn(`TRIWEST-TRICARE Service Dates provider ${provider} returned portal alert without claim rows: ${resultSummary.portalAlertMessage}`);
      lastProviderFailure = `Provider ${provider}: ${resultSummary.portalAlertMessage}`;
      continue;
    }

    if (resultRows.length === 0) {
      logger.warn(`TRIWEST-TRICARE Service Dates provider ${provider} returned no claim rows. Trying next provider if available.`);
      lastProviderFailure = `Provider ${provider}: no claim rows returned.`;
      continue;
    }

    return processTriwestTricareServiceDateResults(page, row, provider, resultSummary, options);
  }

  return {
    status: "failed",
    summaries: [renderFailedSummary(lastProviderFailure || "Claim not found in TRIWEST-TRICARE Service Dates tab for matching Service Date, Charges, and Member ID.")],
    matchCount: 0,
    provider: providerOrder.join(", "),
    sourceTab: "Service Dates",
    notes: lastProviderFailure
      ? `Searched TRIWEST-TRICARE Service Dates providers: ${providerOrder.join(", ")}. Last provider failure: ${lastProviderFailure}`
      : `Searched TRIWEST-TRICARE Service Dates providers: ${providerOrder.join(", ")}. No matching Service Date + Charges + Member ID found.`
  };
}
