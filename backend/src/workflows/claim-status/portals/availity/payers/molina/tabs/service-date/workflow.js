...
async function processMolinaServiceDateResults(page, row, provider, resultSummary, options = {}) {
  ...
  if (matchedRows.length === 0 && matchingPolicy.patientNameFallback && inputMemberId && inputPatientName) {
    logger.info("No Molina Service Dates rows matched Member ID. Applying configured Patient Name fallback.");
    matchedRows = resultRows.filter((result) => {
      return result.serviceDate === inputDate
        && (!shouldMatchBilledAmount || normalizeMoney(result.billedAmount) === inputCharge)
  ...
  if (matchedRows.length === 0 && matchingPolicy.patientNameWithoutInitialFallback && inputPatientNameWithoutInitial) {
    logger.info("No Molina Service Dates rows matched exact Patient Name. Applying configured trailing-initial fallback.");
    matchedRows = resultRows.filter((result) => {
      return result.serviceDate === inputDate
        && (!shouldMatchBilledAmount || normalizeMoney(result.billedAmount) === inputCharge)
  ...
  const selection = selectMolinaMatchedRows(matchedRows, sourceTab);
  if (selection.notes) {
  ...
  for (let index = 0; index < selection.selectedRows.length; index += 1) {
    ...
    const extracted = await extractMolinaMatchedRow(page, matchedRow, sourceTab);
...
async function searchMolinaServiceDatesWithProvider(page, providerName, rowData, options = {}) {
  ...
  if (options.projectId === "charm") {
    await clearProviderFormIfVisible(page, { context: "Charm Molina Service Dates", logger });
  }
  if (providerPolicySkipsProviderDropdown(options.providerFieldPolicy)) {
    ...
    logger.info(`Molina Service Dates field policy skips Select a Provider. Filling Provider Tax ID "${taxId || "blank"}".`);
    if (!taxId && options.providerFieldPolicy?.providerTaxId?.required) {
      throw new Error("Molina Service Dates field policy requires Provider Tax ID, but the input value is blank.");
    }
    const frame = await clearProviderStateForTaxIdFallback(page, { context: "Molina Service Dates field policy", logger });
    await fillProviderTaxId(frame, taxId);
...
async function processClaim(page, row, options = {}) {
  logger.info("Using Molina workflow: Service Dates tab only.");
  ...
  for (const provider of providerOrder) {
    await searchMolinaServiceDatesWithProvider(page, provider, row.data, options);

    logger.info(`Waiting up to 5 seconds for ${provider} Molina Service Dates results to settle`);
    const resultSummary = await waitForSearchResultsToSettle(page, 5000);
    logger.info(
      `Molina Service Dates provider ${provider} result summary: heading="${resultSummary.headingText || "not found"}", total=${resultSummary.total ?? "unknown"}, rows=${resultSummary.resultRowCount ?? "unknown"}, no_results_message=${resultSummary.noResultsMessageVisible}, alert="${resultSummary.portalAlertMessage || ""}"`
    );

    const resultRows = await getMolinaServiceDateRows(page);
    if (resultSummary.hasPortalAlert && resultRows.length === 0) {
      logger.warn(`Molina Service Dates provider ${provider} returned portal alert without claim rows: ${resultSummary.portalAlertMessage}`);
      lastProviderFailure = `Provider ${provider}: ${resultSummary.portalAlertMessage}`;
      continue;
    }

    if (resultRows.length === 0) {
      logger.warn(`Molina Service Dates provider ${provider} returned no claim rows. Trying next provider if available.`);
      lastProviderFailure = `Provider ${provider}: no claim rows returned.`;
      continue;
    }

    return processMolinaServiceDateResults(page, row, provider, resultSummary, options);
  }

  return {
    status: "failed",
    summaries: [renderFailedSummary(lastProviderFailure || "Claim not found in Molina Service Dates tab for matching Service Date, Charges, and Member ID.")],
    matchCount: 0,
    provider: providerOrder.join(", "),
    sourceTab: "Service Dates",
    notes: lastProviderFailure
      ? `Searched Molina Service Dates providers: ${providerOrder.join(", ")}. Last provider failure: ${lastProviderFailure}`
      : `Searched Molina Service Dates providers: ${providerOrder.join(", ")}. No matching Service Date + Charges + Member ID found.`
  };
}
