...
async function selectServiceDateTab(page) {
  await withRetry(
    "Selecting Health Net Service Dates tab",
    async () => {
...
async function selectProvider(page, providerName) {
  await withRetry(
    `Selecting Health Net provider ${providerName}`,
    async () => {
      const frame = await getClaimStatusFrame(page);
...
async function submitServiceDateSearch(page) {
  ...
  await withRetry(
    "Submitting Health Net Service Dates search",
    async () => {
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        ...
        logger.info(`Health Net Service Dates Search clicked (attempt ${attempt}/3). Waiting for portal response.`);
        await humanDelay(1500, 2500);

        if (await resultIndicatorAppeared(5000)) {
          logger.info(`Health Net Service Dates search response appeared after submit attempt ${attempt}.`);
          return;
        }

        if (attempt < 3) {
          logger.warn(`Health Net Service Dates search results did not appear within 5 seconds after submit attempt ${attempt}. Re-clicking Search.`);
        }
      }

      throw new Error("Health Net Service Dates Search did not produce results, no-results message, or validation response after 3 attempts.");
    },
...
async function getHealthNetServiceDateRows(page) {
  const frame = await getClaimStatusFrame(page);
...
function selectHealthNetMatchedRows(matchedRows, sourceTab) {
...
async function extractHealthNetMatchedRow(page, matchedRow, sourceTab) {
  logger.info(
...
async function processHealthNetServiceDateResults(page, row, provider, resultSummary, options = {}) {
  const sourceTab = "Service Dates";
  const resultRows = await getHealthNetServiceDateRows(page);
  ...
  if (matchedRows.length === 0 && matchingPolicy.patientNameFallback && inputMemberId && inputPatientName) {
    logger.info("No Health Net Service Dates rows matched Member ID. Applying configured Patient Name fallback.");
    matchedRows = resultRows.filter((result) => {
      return result.serviceDate === inputDate
        && (!shouldMatchBilledAmount || normalizeMoney(result.billedAmount) === inputCharge)
  ...
  if (matchedRows.length === 0 && matchingPolicy.patientNameWithoutInitialFallback && inputPatientNameWithoutInitial) {
    logger.info("No Health Net Service Dates rows matched exact Patient Name. Applying configured trailing-initial fallback.");
    matchedRows = resultRows.filter((result) => {
      return result.serviceDate === inputDate
        && (!shouldMatchBilledAmount || normalizeMoney(result.billedAmount) === inputCharge)
  ...
  logger.info(`Matched ${matchedRows.length} Health Net Service Dates result row(s) by ${matchLabel}`);
  ...
  const selection = selectHealthNetMatchedRows(matchedRows, sourceTab);
  if (selection.notes) {
  ...
  for (let index = 0; index < selection.selectedRows.length; index += 1) {
    ...
    const extracted = await extractHealthNetMatchedRow(page, matchedRow, sourceTab);
...
async function searchHealthNetServiceDatesWithProvider(page, providerName, rowData, options = {}) {
  logger.info(`Health Net Service Dates provider attempt: ${providerName}`);
...
async function processClaim(page, row, options = {}) {
  logger.info("Using Health Net workflow: Service Dates tab only.");
  ...
  for (const provider of providerOrder) {
    await searchHealthNetServiceDatesWithProvider(page, provider, row.data, options);

    logger.info(`Waiting up to 5 seconds for ${provider} Health Net Service Dates results to settle`);
    const resultSummary = await waitForSearchResultsToSettle(page, 5000);
    logger.info(
      `Health Net Service Dates provider ${provider} result summary: heading="${resultSummary.headingText || "not found"}", total=${resultSummary.total ?? "unknown"}, rows=${resultSummary.resultRowCount ?? "unknown"}, no_results_message=${resultSummary.noResultsMessageVisible}, alert="${resultSummary.portalAlertMessage || ""}"`
    );

    const resultRows = await getHealthNetServiceDateRows(page);
    if (resultSummary.hasPortalAlert && resultRows.length === 0) {
      logger.warn(`Health Net Service Dates provider ${provider} returned portal alert without claim rows: ${resultSummary.portalAlertMessage}`);
      lastProviderFailure = `Provider ${provider}: ${resultSummary.portalAlertMessage}`;
      continue;
    }

    if (resultRows.length === 0) {
      logger.warn(`Health Net Service Dates provider ${provider} returned no claim rows. Trying next provider if available.`);
      lastProviderFailure = `Provider ${provider}: no claim rows returned.`;
      continue;
    }

    return processHealthNetServiceDateResults(page, row, provider, resultSummary, options);
  }

  return {
    status: "failed",
    summaries: [renderFailedSummary(lastProviderFailure || "Claim not found in Health Net Service Dates tab for matching Service Date, Charges, and Member ID.")],
    matchCount: 0,
    provider: providerOrder.join(", "),
    sourceTab: "Service Dates",
    notes: lastProviderFailure
      ? `Searched Health Net Service Dates providers: ${providerOrder.join(", ")}. Last provider failure: ${lastProviderFailure}`
      : `Searched Health Net Service Dates providers: ${providerOrder.join(", ")}. No matching Service Date + Charges + Member ID found.`
  };
}
