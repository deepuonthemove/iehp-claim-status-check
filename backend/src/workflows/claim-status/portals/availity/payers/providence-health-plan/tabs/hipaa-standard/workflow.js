...
const PROVIDENCE_HEALTH_PLAN_PROVIDER_ORDER = ["DAO, THUAN DUC", "TRINITY PAIN MANAGEMENT"];

async function processClaim(page, row, options = {}) {
  logger.info("Using Providence Health Plan workflow: HIPAA Standard search only.");
  const { hipaaAvailable } = await waitForSearchTabs(page, 3000, { preferHipaa: true });
  logger.info(`Providence Health Plan workflow tabs detected: hipaa_standard=${hipaaAvailable}`);

  if (!hipaaAvailable) {
    return {
      status: "failed",
      summaries: [renderFailedSummary("HIPAA Standard tab is not available for Providence Health Plan payer workflow.")],
      matchCount: 0,
      provider: "",
      sourceTab: "",
      notes: "Providence Health Plan workflow requires HIPAA Standard tab, but it was not visible."
    };
  }

  const providerOrder = Array.isArray(options.providerOrder) && options.providerOrder.length
    ? options.providerOrder
    : PROVIDENCE_HEALTH_PLAN_PROVIDER_ORDER;
