...
async function selectServiceDateTab(page) {
  await withRetry(
    "Selecting Humana Service Dates tab",
    async () => {
...
async function selectProvider(page, providerName) {
  await withRetry(
    `Selecting Humana provider ${providerName}`,
    async () => {
      const frame = await getClaimStatusFrame(page);
...
async function submitServiceDateSearch(page) {
  ...
  await withRetry(
    "Submitting Humana Service Dates search",
    async () => {
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        const frame = await getClaimStatusFrame(page);
        await throwIfVisibleFieldValidation(page, "Humana Service Dates");
        ...
        logger.info(`Humana Service Dates Search clicked (attempt ${attempt}/3). Waiting for portal response.`);
        await humanDelay(1500, 2500);

        if (await resultIndicatorAppeared(5000)) {
          logger.info(`Humana Service Dates search response appeared after submit attempt ${attempt}.`);
          return;
        }

        if (attempt < 3) {
          logger.warn(`Humana Service Dates search results did not appear within 5 seconds after submit attempt ${attempt}. Re-clicking Search.`);
        }
      }

      throw new Error("Humana Service Dates Search did not produce results, no-results message, or validation response after 3 attempts.");
    },
...
async function getHumanaServiceDateRows(page) {
  const frame = await getClaimStatusFrame(page);
...
function selectHumanaMatchedRows(matchedRows, sourceTab) {
...
async function extractHumanaMatchedRow(page, matchedRow, sourceTab) {
  logger.info(
...
async function processHumanaServiceDateResults(page, row, provider, resultSummary, options = {}) {
  ...
  if (matchedRows.length === 0 && matchingPolicy.patientNameFallback && inputMemberId && inputPatientName) {
    logger.info("No Humana Service Dates rows matched Member ID. Applying configured Patient Name fallback.");
    matchedRows = resultRows.filter((result) => {
      return result.serviceDate === inputDate
        && (!shouldMatchBilledAmount || normalizeMoney(result.billedAmount) === inputCharge)
  ...
  if (matchedRows.length === 0 && matchingPolicy.patientNameWithoutInitialFallback && inputPatientNameWithoutInitial) {
    logger.info("No Humana Service Dates rows matched exact Patient Name. Applying configured trailing-initial fallback.");
    matchedRows = resultRows.filter((result) => {
      return result.serviceDate === inputDate
        && (!shouldMatchBilledAmount || normalizeMoney(result.billedAmount) === inputCharge)
  ...
  const selection = selectHumanaMatchedRows(matchedRows, sourceTab);
  if (selection.notes) {
...
async function processClaim(page, row, options = {}) {
  ...
  for (const provider of providerOrder) {
    await searchHumanaServiceDatesWithProvider(page, provider, row.data, options);
    ...
    logger.info(
      `Humana Service Dates provider ${provider} result summary: heading="${resultSummary.headingText || "not found"}", total=${resultSummary.total ?? "unknown"}, rows=${resultSummary.resultRowCount ?? "unknown"}, no_results_message=${resultSummary.noResultsMessageVisible}, alert="${resultSummary.portalAlertMessage || ""}"`
    );

    const resultRows = await getHumanaServiceDateRows(page);
    if (resultSummary.hasPortalAlert && resultRows.length === 0) {
      logger.warn(`Humana Service Dates provider ${provider} returned portal alert without claim rows: ${resultSummary.portalAlertMessage}`);
      lastProviderFailure = `Provider ${provider}: ${resultSummary.portalAlertMessage}`;
      continue;
    }

    if (resultRows.length === 0) {
      logger.warn(`Humana Service Dates provider ${provider} returned no claim rows. Trying next provider if available.`);
